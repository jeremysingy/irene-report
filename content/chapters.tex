...
Some information, mainly historical, is taken from \autocite{wikigram12}, \autocite{wikiphono12} and \autocite{wikiphonocyl12}.
...
Some information is taken from previous thesis or technical report realized at \gls{lbnl}, such as \autocite{benninger12}, \autocite{heinzer12} and \autocite{haber11}.
...
There are two main different supports. The first one is a turntable specifically designed for flat discs with the scanner put vertically and the disc rotating horizontally. It is viewed in \autoref{fig:labirene} as installed with the 2D camera .

\begin{figure}[!ht]
\centering
\includegraphics[width=0.8\textwidth]{images/hardware-irene}
\caption{The acquisition hardware to acquire discs as installed at LBNL.}
\label{fig:labirene}
\end{figure}

The second one enables cylinder scanning, the probe being placed horizontally and directly facing the record. It can is represented in \autoref{fig:lab3d} as installed laboratory, with the 3D probe.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.8\textwidth]{images/hardware-3d}
\caption{The acquisition hardware to acquire cylinders as installed at LBNL.}
\label{fig:lab3d}
\end{figure}
...
Since each captured line are part of a sequence, an entire revolution finally results in a ring-shaped capture which is mapped in a rectangular image. The groove then maps to straight parallel lines instead of the original spiral. This mapping is more clearly seen in \autoref{fig:discmapping}.
...
A collection of Graham Bell records\footnote{More information on the recovered collection can be found on this IRENE web page: \url{http://bio16p.lbl.gov/volta-release.html}.} has been recovered using the IRENE \gls{3dprobe} at the Library of Congress in Washington DC. This collection comes from the Volta Laboratory \autocite{wikivolta12} created by Alexander Graham Bell. One of the disc in the collection, labeled 287700, is a vertically-cut wax disc. It can be viewed in \autoref{fig:belldisc}.
...
This specific wax cylinder comes from an experimental project by William Dickson and Thomas Edison \autocite{wikidickson12}. It is the soundtrack recording for the first film with recorded sound in addition to the motion picture. It is then an early prototype of a sound-film, but without real synchronization between picture and sound. This first attempt of a sound-film is known as the \emph{Dickson Experimental Sound Film}.
...
Franz Boas was a famous anthropologist born in 1858 in Germany \autocite{wikiboas12}. He moved to the United States in 1887 where he started to study Native North American populations. During his researches and expeditions on Vancouver Island in 1930, Boas recorded sound. Some of these records have been acquired with \gls{3dprobe} for restoration some time ago. One of the disc in the collection, labeled 0724, is a damaged wax cylinder.
...
%An example can be viewed in \autoref{fig:intassistedtrack}.
...
\caption[Panel and record coordinate systems in the interactive tracking panel.]{Panel and record coordinate systems in the interactive tracking panel. The first one is represented in orange and the second in green. When the record is drawn at $(30,200)$, the bitmap is translated to the left and top.}
...
\subsection{Groove center approximation}
...
However, at this point, the tracking is just a rough approximation, and the real useful values are the top edges delimiting the groove. It will then be sufficient to keep track the brightest point in the neighborhood of the part pointed by the user to improve the tracking. The curve fitting method is then irrelevant for \gls{rene} and the option is removed.
...
The mathematical operation for applying a filter is called a convolution. In the case of an image, the source can be viewed as a matrix. The new image is computed by applying a convolution matrix, also called \emph{kernel}, which contains different weights. Convolution then replaces one pixel value with a new value determined as a function of a group of pixels. The size of the group is determined by the kernel size and the (linear) coefficients are the kernel values. It is also possible for the function to be non-linear but that can be considered as a special case. For the linear this means
\begin{equation}
\label{eq:convolution}
b^{\ast}_{mn} = \sum_{i=m-a,j=n-b}^{m+a,n+b} K_{ij} p_{ij}
\end{equation}
...
With the mathematical notation $\ast$, not to be confused with matrix multiplication, the equation for a $3 \times 3$ kernel would then simply become
\begin{equation}
...
If the coefficients of $K$ were all the same in \eqref{eq:convolsimple}, the kernel would result in a simple blur consisting of the average of all pixel values around. The dimensions of a convolution matrix must be an odd value but it is not necessarily square.
...
A common tool helping for the segmentation operation is the histogram. A histogram represents the statistical distribution of a variable. Therefore, computing the histogram of an image means to compute the number of occurrences of each pixel values (for example in the range \numrange[range-phrase=--]{0}{255} for an 8-bit greyscale image). In our case this corresponds to the brightness intensity.
...
\end{axis}
\end{tikzpicture}
\caption{Histogram of a portion of the cracked record.}
\label{fig:histocrack}
\end{figure}

This histogram shows clearly different peaks on the intensity value. The first peak represents very dark pixel values. As predicted, this is the intensity inside the crack area, which is clearly detached from the rest of the image. The crack is approximately represented by the intensity between \numrange[range-phrase=--]{0}{10} and the rest of the image in the intensity \numrange[range-phrase=--]{11}{255}.
...
Morphological operations are a technique to clean an image based on the pixels in the neighborhood. These are then boolean masks applied on the binary image in a way similar to the filtering operations seen before. The theoretical explanations in this section are partly inspired from \autocite{wikimorpho12}.
...
In the beginning, several edge detection algorithms have been tested, e.g. the well-known Sobel and Canny filters. In the end, they were not suitable for our purpose. The thresholding as detailed in \autoref{sec:chunkthresh} (\autopageref{sec:chunkthresh}) is the only algorithm that has been implemented in \gls{prism}.
...
In \gls{visualaudio}, even if the cracks are much bigger, this issue may also happen. The workaround is to manually edit the exported image to ``draw'' the missing part. In the case of \gls{prism}, we will try to avoid it, and the frontier detection (as it will be explained in \autoref{sec:findfrontiers}) should not be affected by this issue.
...
At this point, the choice has been made not to directly follow the idea of \gls{visualaudio}. In the latter, the next step after the chunks detection is directly the tracking (or even processing, as there is no previous tracking step as with \gls{irene}). The portion inside the chunk is isolated by applying a mask, and the grooves are found and tracked by a method using histograms (see Chapter~6 in \autocite{seydoux08}). It is only after this step that the chunks (now defined by a set of traces, or grooves) are separated in frontiers to continue towards the linking step.

This choice was made after analyzing the \gls{visualaudio} code and documentation. This part was quite complex and it required some algorithms that did not look relevant in the case of \gls{prism}. Instead, it was decided to geometrically separate the chunk into frontiers, before finding and tracking the groove from the detected frontiers.
...
\centering
    ...
    \end{subfigure}
    \caption[Relation between peak and frontier detection.]
    {Relation between peak and frontier detection inside a chunk. The extremities of each frontiers are numbered in both cases and correspond to the extrema on the resulting signal (right side).}
...
\subsubsection{Slope correction}
...
\begin{figure}[!ht]
\tikzsetnextfilename{slope-correction}
\centering
\begin{tikzpicture}
\begin{axis} [ unit vector ratio*=2 1 1, width=0.7\textwidth,
legend style={at={(1.1,0.5)},anchor=west}]%draw=none
%,xlabel=x,ylabel=value,xmin=0, xmax=2
\addplot +[mark=none] table[x expr=\coordindex,y expr=0.1*\thisrow{raw}-500, restrict x to domain=0:600] {graphs/slope-correction.dat};
\addplot +[mark=none] table[x expr=\coordindex,y expr=0.1*(\thisrow{slope}+1585), restrict x to domain=0:600] {graphs/slope-correction.dat};
\addplot +[mark=none] table[x expr=\coordindex,y expr=0.1*(\thisrow{new}-6585), restrict x to domain=0:600] {graphs/slope-correction.dat};
\legend{Original, Lin. regression, Subtracted};
\end{axis}
\end{tikzpicture}
...
\lstinputlisting[language={[Sharp]C},
caption={Pseudocode representing localization of the end position and frontier.},
...
Finally, the process repeats for every groove in the frontier. As long as the closest frontier remains the same, the section is conceptually enlarged by adding a groove. If the closest frontier changes, it means that a new section must be defined. The process then continues similarly. A full example showing the result of this processing is presented in \autoref{fig:frontlinkex}.
...
In specific situations, e.g. when the crack is almost vertical (see \autoref{fig:crackbellprob}), it is possible that the next frontier is slightly above the end groove position. A small negative number is then allowed. The typical value is for example 5 pixels, so that it would not be possible for a wrong frontier to be detected (the chunk should have a height less or equal to 5 pixels). The algorithm is described in \autoref{lst:frontierclosest}.
...
The classes involved in this step are listed in \autoref{fig:frontlinkdiag}.

\begin{figure}[!ht]
\centering
\includegraphics{diagrams/frontier-linking.1}
\caption{Class diagram for frontier linking.}
\label{fig:frontlinkdiag}
\end{figure}

The \texttt{FrontierLinker} class contains the algorithms previously explained and will link all frontiers together. The \texttt{Link()} method takes all frontiers in parameters and builds a list of \texttt{FrontierSection} objects that is directly returned.
...
Bad values are replaced with a interpolation of the neighbors. The result of this algorithm can be visualized in \autoref{fig:removeout}. In this case, especially one big outlier is visible that is removed after the cleaning.
...
The next sections will explain the different characteristics that will be computed over this audio signal. Some of the implemented techniques are derived from the work in \gls{visualaudio}, from \autocite{seydoux08} and \autocite{benningerchunk12}.
...
\begin{figure}[!ht]
\tikzsetnextfilename{fft-filter-ex}
\centering
\begin{tikzpicture}
\begin{axis} [width=0.7\textwidth, unit vector ratio*=1 0.7 1, xticklabels={,,},
legend style={at={(1.05,0.5)},anchor=west}]
\addplot +[mark=none] table[x expr=\coordindex,y expr=\thisrow{yraw}-271] {graphs/fft-filter-ex.dat};
\addplot +[mark=none] table[x expr=\coordindex,y=yfiltered] {graphs/fft-filter-ex.dat};
\legend{Without filter, With high-pass filter};
\end{axis}
\end{tikzpicture}
...
\begin{figure}[!ht]
\tikzsetnextfilename{windows-comp}
\centering
\begin{tikzpicture}
\begin{axis} [width=0.8\textwidth, ymax=3000, xlabel=Frequency,ylabel=Magitude %, unit vector ratio*=1 0.7 1, xticklabels={,,} %,
%legend style={at={(1.05,0.5)},anchor=west}]
]
...
%TODO is it explained with the threshold for indicating the user?
...
To match the frontiers from the computed characteristics, the method \texttt{BestMatching()} takes the maximum allowed shift (e.g. if 3, it looks from \numrange{-3}{3}), tries all positions and sets the shift corresponding to the best match. The likelihood is computed on the maximum number of possible groove sections and averaged before being compared. The comparison ratio between two grooves regarding the different used characteristics is explained in the following sections.

\subsubsection{Using standard deviation}

Comparing two standard deviation is quite easy. The implementation is based on what is done in \gls{visualaudio}, especially from the results in \autocite{benningerchunk12}. A ratio $\sigma i_{rat}$ is computed between the standard deviations of two groove sections $\sigma 1$ and $\sigma 2$ with:
\begin{equation}
...
An \gls{fft} comparison has however been implemented, which basically compares the overall difference between the two results. As explained in \autoref{sec:audiochar}, the stored spectrum is already filtered to remove very low and high frequencies, so the size is also the same for every sections. A formula to get the normalized distance between two signals $\mathbf{s_1}$ and $\mathbf{s_2}$ is then
\begin{equation}
\label{eq:fftcomp}
...
\begin{figure}[!ht]
\centering
\includegraphics{diagrams/audio-characteristics.1}
...
%TODO stop beyond frontiers?

\subsubsection{Overview}

Firstly, the sections in a same revolution are linked together. The class \texttt{FrontierSection} embeds a function named \texttt{LinkGrooves()}, which iterates through all grooves in it and, following the appropriate shift, links them together by setting the  \texttt{NextGroove} property. At this point every groove section forming revolution is linked.

Then, the grooves section known as \emph{starting} sections must be detected. This is done by keeping track of all frontiers sections that are not linked to other ones, meaning that they are located at the start of a revolution, i.e. at the top of the capture. At that time, they are also sorted by their X-coordinate to ensure the continuous tracking.

The very final step is the iteration over the starting sections. For each of them, the tracked points are added and the next groove followed until a revolution is reached. Then, it continues to the next starting sections until the whole record is scanned. The corresponding algorithm is summarized in \autoref{lst:trackbuild}.

\lstinputlisting[language={[Sharp]C},
caption={Algorithm for final track building.},
label={lst:trackbuild}]
{listings/track-building.cs}

One can note that this algorithm continues to link from the starting grooves without actually verifying if the end of the revolution matches correctly. In fact, this is a willing behavior. As with other tracking methods, the result will be shown to the end-user. This enables to clearly identify if the match is correct, i.e. if the links between the bottom to the top are vertical (they should have approximately the same horizontal position). Otherwise, the user can use the interface to easily correct the matches (see \autoref{chap:usermanauto}).
...
\begin{figure}[!ht]
\centering
\includegraphics{diagrams/groove-linking.1}
...
\begin{figure}[!ht]
\centering
\includegraphics{diagrams/crack-proc-integration.1}
...
%TODO add general information/considerations

\chapter{Tests on sample recordings}
\label{chap:tests}

This chapter presents an the results of the final tests that have been performed in order to test that the final solution is functional and behaves in a natural way for the end-user. The first section will explain the tests and results about the interactive tracking whereas the next one will talk about the automatic processing

The sample recordings will be used to apply the tests. For the interactive tracking, the records will not be entirely processed, as the goal is to specifically test the user experience. The exact parameters used for the test as well as the final resulting audio may be found in the external files (see \autoref{chap:arch-content}).

\section{Interactive tracking}

This test corresponds to a \gls{gui} test. The goal is to verify that the behavior corresponds to the user expectation with different situations. The tests are applied on all records except the Bell disc, because the result would be similar, and it has also been successfully processed with the automatic tracking (see \autoref{sec:testautotrack}). The two first tests are done with the implementation in \gls{prism} and the last one (Brigance) with \gls{rene}.

%\subsection{Graham Bell disc}

%Tracking this record is quite easy with the interactive tracking. The difficult point for the user is to look for the correct match. Once it is found, the pattern is easily copyable and the record is quickly tracked. A part of the record is viewed in \autoref{

\subsection{Dickson cylinder}

Tracking this record is quite easy with the interactive tracking. On the main part, there is only one big crack and scratch, but the shift is not big and easily findable. Once the pattern is found, it may be copy multiple times, to that the record is quickly tracked. A part of the result is viewed in \autoref{fig:testintdickson}.

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\textwidth]{images/test-int-dickson}
\caption{Test of interactive tracking on the Dickson cylinder.}
\label{fig:testintdickson}
\end{figure}
...
The final sound quality is correct though the volume is quite low. The music is audible but the speech difficult to understand. In this test, it has not been tried to optimize the processing part. The blob correction feature has been used to minimize the noise.

\subsection{Boas recording}

This record is more difficult to track. The main point is to find the correct shift between the cracks, which is quite tricky in the first place. However, as usual, when a first revolution is correctly tracked, tracking the rest of the record is quite easy. An overview or this record correctly tracked is presented in \autoref{fig:testintboas}.

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\textwidth]{images/test-int-boas}
\caption{Test of interactive tracking on the Boas recording.}
\label{fig:testintboas}
\end{figure}

The first run (beg 3) of has been recovered. The tracking has been applied in two different steps, from range 1 to 6 and 6 to 10. It seems to be a recording of a singing with percussions, probably recorded from a Native American population. Because of noise, the final quality is quite bad.

\subsection{Brigance recording}

This record has been used to test the version of the interactive tracking implemented in \gls{rene}. The result of the tracking is good, as viewed in \autoref{fig:testintbrigance}.

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\textwidth]{images/test-int-brigance}
\caption{Test of interactive tracking on the Brigance recording.}
\label{fig:testintbrigance}
\end{figure}

The audio result is a speech. However, the sound is very noisy. As explained in \autoref{sec:samplebrigance}, the record also has some bubbles on the its surface. Therefore, the processing step is not able to find properly the edges when the groove bottom is blurred.

\subsection{Summary}

These tests enabled to show that the interactive tracking works well in different situations to help the user tracking some records. All tests worked properly and in the end gave an audio result. Of course, it would take some more time to improve the sound quality, by tuning the right options.

Also, as a result from these tests, some future useful improvements for this feature have been identified. These will be explained in \autoref{sec:improvements}.

\section{Automatic tracking}
\label{sec:testautotrack}

The second main part of the project was about developing a solution able to automatically track some cracked records. From the implementation, a lot of different issues have been identified. The two most difficult parts are the crack and groove detection. If the cracks or the grooves are not properly detected, the whole mechanism cannot work. Indeed, the link cannot match if a groove is missing, as seen in \autoref{fig:missinggroove}.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.5\textwidth]{images/grooves-missing}
\caption[Examples of grooves not detected on a frontier.]
{Examples of grooves not detected on a frontier. This makes the linking unusable.}
\label{fig:missinggroove}
\end{figure}
...
Finally, a linking based only on audio comparison is not precise enough in all cases. It is quite good when the comparison occurs between silent parts and well-formed audio. However, if the audio remains similar or silent, the user has to fix the final linking. In most cases it is quite easy for a human to fix it, using by viewing the geometry. 

%This section will present different results from the
%Some have been fixed, but the main problem 
%In the end, the new feature is able to track some records but not 

\subsection{Bell disc}

This record has been entirely restored using automatic tracking. The cracks on this record are detected quite easily because the brightness is very different than the regular surface (see \autoref{fig:belldisc}). Also, all the grooves are found because they are deeply embossed to the surface. However, the binned image must be transformed with the fixing algorithms (see \autoref{sec:binimagecorr}) because the surface height is very changing, both because of the scan (probe not aligned) and the cracks (the surface is altered when it occurs).

The result after restoring the second part of the record is viewed in \autoref{fig:testautobell}.

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\textwidth]{images/test-auto-bell}
\caption[Tracking of the Bell disc using automatic tracking.]
{Tracking of the Bell disc using automatic tracking.}
\label{fig:testautobell}
\end{figure}

The record has been recovered in three steps, as it have four separated tracks recorded. A talking man can be heard on all these parts, though it is difficult to understand what he says because of the high noise level. However the tracking is consistent for all parts in this example.

\subsection{Dickson cylinder}

This record was more problematic to track automatically. In fact, the cracks are pretty well detected, but the big scratches result in weirdly-shaped chunks, making the tracking very difficult. Moreover, the grooves on this cylinder are very weakly engraved and quite thin, which makes groove detection and tracking more difficult.

In the end, with proper parameters, although almost all grooves are correctly tracked, the linking step does not work and the automatic tracking is unusable. An overview of the result in a section of the acquisition is presented in \autoref{fig:testautodickson}.

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\textwidth]{images/test-auto-dickson}
\caption[Tracking of the Dickson cylinder using automatic tracking.]
{Tracking of the Bell disc using automatic tracking.}
\label{fig:testautodickson}
\end{figure}

The first pretty good tracked. The main problem comes with the big scratch. As seen in the detailed view (part right), the surface at this position is completely distorted, without any groove seen, which makes the linking impossible.

\subsection{Boas recording}

Another test was done on the Boas cylinder. The results are quite similar to the Dickson one. Again, even with very thin cracks the detection looks good, but some missing grooves and bad value because of ill-shaped cracks give an unusable result, as seen in \autoref{fig:testautoboas}.

\begin{figure}[!ht]
\centering
\includegraphics[width=1.0\textwidth]{images/test-auto-dickson}
\caption[Tracking of the Boas cylinder using automatic tracking.]
{Tracking of the Boas cylinder using automatic tracking.}
\label{fig:testautoboas}
\end{figure}
...
These different tests have shown that the interactive is an interesting feature to help the user tracking any types of records, even if they are very damaged. The new features enhance the productivity by providing ``shortcuts'' to the user.

On the other side, the automatic tracking, even if working on specific records, is not a tool appropriated in the general case. In fact, each internal function -- cracks detection, chunk detection, audio comparison -- represents a new ``brick'' that may help tracking a record. However, put together, it is very difficult to get an appropriate result in every situation.

This leads to the idea of a possible solution that would stand in between a fully manual or automatic system. Specific parts that are problematic could be done or fixed manually by the user, like draw a chunk border by hand, indicating missing grooves or fix a bad tracking. These improvements will be explained more specifically in \autoref{sec:improvements}.

%The interactive tracking is already a step towards automation.
...
The main objectives of the project are detailed in \autoref{sec:mainobjectives}.

\subsection{Analysis}

The first main objective was about analyzing the existing programs and becoming familiar with the codebase and the main algorithms involved. After a phase of pure code analysis, the majority of the code was gradually studied while adding the first new features, with the early tests of the interactive tracking feature.

An analysis of the cracked records algorithm in \gls{visualaudio} was also made, but more theoretically by studying \autocite{seydoux08}. Later in the project, the differences between the systems showed that although the basic idea could be used, the implementation would be quite different and require specific solutions.

\subsection{Implementation}

Then, the main part of the project was to find different ways to restore cracked record in an efficient way. The work to improve the manual tracking led the new interactive tracking, adding different features which proved to be very useful for the user. The feature has been implemented bot in \gls{rene} for 2D acquisitions and in \gls{prism} for 3D.

As a further step, the next goal was to find a way of automatically processing some damaged records, as it is possible in certain situations with \gls{visualaudio}. The final implementation is a complete system which tries to find the cracks for tracking damaged records. This solution works in some cases but may still be improved in several ways (see below).

Regarding the available recordings and the types of damages, only an implementation for \gls{prism} has been completed. The specific damages on Brigance disc, the only tested 2D acquisition, were almost impossible to find with image processing. Moreover, it suffers from other problems such as focus and is easily tracked with the interactive tracking.

About integration and avoiding regression, the new features have been implemented as separate as possible from the rest of the program, so that it is easily maintainable (for example with the use of a facade). A secondary objective was to improve the software architecture. Some technical details have been improved but for a better result, a deep software reengineering should be performed, which was out of the scope of this thesis.

\section{Future work and improvements}
\label{sec:improvements}
%This section , a lot of different problems occurred and had to be fixed. Of course, not everything can be detailed but here is a list summarizing the mains encountered issues.

The final results from the researches conducted during this thesis open the way to many possible future work. Not only some specific points regarding the implementation may be improved but also some new ideas emerged given the current stage. Some of these are explained in the next sections.

\subsection{Interactive tracking}

The interactive tracking already offers some useful new features. While tracking records, other ideas came to improve it. Firstly, it could be useful to be able to fix an area already tracked, without being forced to cancel everything after it. One could imagine a way to move or remove an existing point by clicking on it.

Another improvement would be a way to ``jump'' over a crack ar a part without audio, that means, to be able to cut off the tracking in several pieces (with a result similar to what is done in the automatic tracking). Until now, the path is always continuous and the user just passes through the cracks. In the final audio, this often results in some heavy clicks adding disturbing noise.

As a detail, one can also note that in the final implementation the groove center is always automatically fixed (using fitting or depth correction). In some bad areas, the user might want to set the position exactly. It could also be possible to set the maximum deviation (now fixed from the groove width).

\subsection{Cracks and groove detection}

The cracks and groove detection were the most problematic parts during the implementation. Particularly for groove detection, there is a lot of other possible things. The peak detection followed by a cleaning pass is not really satisfactory. In some cases, it could be better to base the detection of the grooves using e.g. the Fourier analysis in a way similar to the existing Fourier tracking. Knowing the phase from the groove signal could give results less sensitive to outliers. 

Regarding the cracks detection, one possibility is also to let the user draw the chunk borders, for example on the binned image. On certain records, the main chunks may be obvious to the human eye. In fact, an early test in this direction has already been tested in the end of the project. In the \texttt{CracksProcessor} class, a method called \texttt{ProcessTestManual()} has been implemented, enabling the create some chunk (for now directly from the code).

In a step beyond, one could imagine to let the user fox or do manually the parts giving wrong results, and let the original code do the rest automatically. The current implementation is flexible enough to be used from any step to any other.

\subsection{Tracking in chunks}

The implemented tracking methods inside a chunk could be also improved. It would also be useful to detect when a groove is mingled to another, warning that one of the initial groove is badly tracked.

\subsection{Linking and audio analysis}

The audio analysis is not perfect in any case. It would be possible to improve it by improving the signal creation or the comparison formulas. For example, the high-pass filter has been implemented from scratch. Particularly, it sets all coefficients to zero until the cutoff frequency is reached. A better way would be to use a common filter like Butterworth.

Also, another way would be to take a totally different approach. In essence, analyzing the audio is not sufficient for comparing grooves. One could imagine a solution using the geometry, e.g. if two grooves are the first ones from the record border, they should probably be linked together.
