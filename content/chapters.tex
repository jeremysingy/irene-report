...
\item Tracking only on the general image, which can be laborious to be clearly seen, especially if the scanned part is big and the record is cracked or damaged.
...
The ability to undo what was done so far to be able to correct a wrong decision is a useful feature. Likewise, it could be gainful to be able to put some annotations before actually track the grooves. \gls{rene} and \gls{prism} offer already information and statistics about the scanned record. For example, one could notice that the signal on the graph between two shifted grooves is similar. It would then be possible to click on the specific ends to indicate that they are likely to be linked together for the tracking.
...
As the current interface is already overflowed with all existing parameters and options, it is important to add the new feature so that it is integrated smoothly in the existing application, without make it tedious to use.

As seen in \autoref{fig:prismgui}, the interface is already able to show a detailed part of the disc on the right panel. When the user click on this detailed part, he can visualize other information on the signal. However, it is not designed to be interactive. In fact, each time the user select a part on the right image, the corresponding part is reloaded and stored in a bitmap to be drawn on screen. The whole process is too much time-consuming for an interactive drawing though it is suitable for its purpose.
...
The controls enable to control the tracking by changing several parameters such as the speed or the zoom level. As explained in \autoref{sec:inttrack}, the zoom can also be locked regarding to the speed, using the corresponding option.

\subsection{Principle of operation}

To use the interactive tracking, the user first selects the correspond button in the tracking options. Then, when the record is loaded, the corresponding disc is visible in the interactive panel. A red triangle acts as a beacon to clearly see the current tracked position.

When the user click on the panel and move the cursor, the horizontal position moves accordingly. The goal is then to place the triangle tip on the center of the groove. While maintaining the mouse button pressed, the user may then increase the speed by using the mouse wheel. The record vertically while the horizontal position is still following the cursor.
...
The reverse is also true. When the left panel is clicked, the position in the interactive panel is changed accordingly. When the tracking is running (the \emph{Play} button is pushed), click on the panel has also the effect of adding an interpolation point. In fact, this acts at the same way as the original manual tracking. It is useful for some areas where the disc is in good quality and pass through the entire groove interactively would be too long. The user can easily switch between the two alternatives while tracking a record.

\subsubsection{Undo option}

One of the problems that happens with the manual tracking is the lack of an option to undo the last actions. It appears quite frequently that the last action is improper resulting in a back tracking. The interactive tracking offers the possibility to undo the steps up to the starting point.

\subsection{Design and classes organization}

This part of the implementation is much about user interface and does not require a lot of processing. The main point is to find an appropriate way of integrating the new feature without blowing up the existing codebase which is already quite complex.
%TODO design of application in chapter about systems?

The new feature has been implemented essentially in two new classes called \texttt{InteractiveControl} and \texttt{DrawPanel} as represented in the diagram of xx.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.8\textwidth]{images/int-track-prism-diag}
...
To perform an animation, one may then use a \texttt{Timer} (provided in the \gls{api}) and, at each tick, update the view before invalidate the panel. The interval should be set to a value giving a refresh rate high enough. For example, with an interval of \SI{40}{\milli\second}, the corresponding refresh rate is $1/0.04 = \SI{25}{fps}$.

%TODO: Another consideration is the transformations. The stored information for the current position is, from an external point of view, the coordinates in the record. 
...
%TODO finding grooves, etc.

%TODO
%\chapter{Automatic scanning} OR
%\chapter{Automatic reassembly}
