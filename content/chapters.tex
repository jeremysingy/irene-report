...
For the records processed with \gls{3dprobe}, an early response to this problem is the information of the brightness intensity reflected by the probe. Lets analyze what looks like a crack in terms of brightness. This first example in \autoref{fig:crackdepthbri} comes from the Graham Bell disc.
...
Though it is difficult to see a clear difference in the first image, on the second one the pixels are much darker on the crack area. This happens in this case because the material of the underneath support reflects less brightness than the normal surface.

Of course, in the general case the intensity depends on the material. For example, in a metallic disc, the brightness would be much higher instead. However, in all the samples used in this project from \gls{3dprobe}, the reflected brightness is clearly lower than the surface. This property will then be used to find the chunks.

\subsection{Filtering}

When detecting a features on a digitalized image, the first task is to apply a filter to eliminate the noise present in the image. Typically, what is needed is a way of smoothing (blurring) the image.

\todo{complete section}

%Because we will use the brightness intensity, reflected as the pixel intensity in the image
