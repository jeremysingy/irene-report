...
legend style={cells={anchor=west},at={(1.1,0.5)},anchor=west}]%draw=none
%,xlabel=x,ylabel=value,xmin=0, xmax=2
...
This slope correction may be activated by the user by selecting a new option in the \gls{gui} (see \autoref{chap:usermanauto}).

\subsubsection{Subtract average}
...
\begin{tikzpicture}
\begin{axis}[width=0.6\textwidth,
legend style={at={(1.1,0.5)},anchor=west, cells={anchor=west}}]%draw=none
%,xlabel=x,ylabel=value,xmin=0, xmax=2
...
\centering
\begin{tikzpicture}
\begin{axis} [width=0.8\textwidth, ymax=3000, xlabel=Frequency,ylabel=Magnitude,
legend style={cells={anchor=west},draw=none}]
]
...
In \gls{visualaudio}, there is also a notion of \emph{reliability}. The reliability is a computed value used to determine whether the shift may be fixed as-is or if the user should review the linking. In the current implementation, given the first results, it was decided to show all linkings to the user and therefore the reliability is not calculated. This way, the user may fix any previously computed linking.
...
\subsection{Integration in the existing software}
...
Tracking this record is quite easy with the interactive tracking. On the main part, there is only one big crack and scratch, but the shift is not big and easily findable. Once the pattern is found, it may be copied multiple times, so that the record is quickly tracked. A part of the result is viewed in \autoref{fig:testintdickson}.
...
This record has been used to test the version of the interactive tracking implemented in \gls{rene}. The result for the tracking is good, as viewed in \autoref{fig:testintbrigance}.
...
The assisted tracking (see \autoref{sec:intfeatures}) works also well on this record, centering to the position where the brightness is the highest. The audio result is a speech but the sound is once more very noisy. As explained in \autoref{sec:samplebrigance}, the record also has some bubbles on the its surface. Therefore, the processing step is not able to find properly the edges when the groove bottom is blurred.
...
This record has been entirely restored using this feature. The cracks are detected quite easily because the brightness is very different compared to the regular surface (see \autoref{fig:belldisc}). Also, all the grooves are found because they are deeply engraved to the surface. However, the binned image must be transformed with the fixing algorithms (see \autoref{sec:binimagecorr}) because the surface height is very changing, both because of the scan (probe not aligned) and the cracks (the surface is sometimes altered when it occurs).
...
Another test was done on the Boas cylinder. The results are quite similar to the Dickson one. Again, even with very thin cracks the detection looks good, but some missing grooves and bad values because of ill-shaped cracks give an unusable result, as seen in \autoref{fig:testautoboas}.
...
The main objectives of the project are detailed in the requirements (\autoref{sec:mainobjectives}). A summary of the achieved objectives is presented in the next sections.
...
The main part of the project was to find different ways to restore cracked record in an efficient way. The work to improve the manual tracking led the new interactive tracking, adding different features which proved to be very useful for the user. The feature has been implemented both in \gls{rene} for 2D acquisitions and in \gls{prism} for 3D.

As a further step, the next goal was to find a way of automatically processing some damaged records, as it is possible in certain situations with \gls{visualaudio}. The final implementation is a complete system which tries to find the cracks for tracking damaged records. This solution works in some cases but may still be improved in several ways (see \autoref{sec:improvements}).

Regarding the available recordings and the types of damage, only an implementation for \gls{prism} has been completed. The specific damages on the Brigance disc, the only tested 2D acquisition, were almost impossible to find with image processing. Moreover, this record suffers from more important problems such as focus and is easily tracked with the interactive tracking.
...
Another idea is to be able to ``jump'' over a crack while tracking, that is, to be able to cut off the tracking in several pieces (with a result similar to what is done in the automatic tracking). Until now, the path is always continuous and the user just passes through the cracks. In the final audio, this often results in some heavy clicks adding disturbing noise.
...
Regarding the cracks detection, one possibility is also to allow the user to draw the chunk borders, for example on the binned image. On certain records, the main chunks may be obvious to the human eye. In fact, an early attempt in this direction has already been tested. In the \texttt{CracksProcessor} class, a method called \texttt{ProcessTestManual()} has been implemented, that embeds fixed-shaped chunks. An adaptation of the user interface would allow to interactively create those chunks.
